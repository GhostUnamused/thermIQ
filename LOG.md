...
## task-061 | 2026-07-11 | DONE
Fixed the "link me to graph" chat query producing a broken generic LLM refusal ("I am a text-based AI...") by adding `isPureGraphNavRequest()` — a narrow nav-verb-near-"graph" regex that short-circuits the RAG round-trip entirely for genuine navigation asks, returning a deterministic friendly reply instead. Also fixed the graph-link chip rendering below/colliding with the absolutely-positioned `.msg-actions` row by merging it into the existing `.followup-chips` row instead of appending inside `.chat-bubble`. Added ↑/↓ arrow-key chat input history recall (cycles through the active chat's sent messages, preserves in-progress draft, resets on send/switch/typing). Static verification only (grep-confirmed `isPureGraphNavRequest`, `historyIdx` state, and chip-merge logic all present; `node --check docs/app.js` passed) — live click-testing still needed. Commit: 4eae9e4 (bundled with task-062).

## task-060 | 2026-07-11 | DONE
Replaced the ₹-subtraction "Simulate fix" gimmick (YC: "it's a basic calculator, acts like it's doing something different") with a transparent closure-priority ranking: `computePriority()` blends ₹ magnitude (45%), failure severity (20%), outage-evidence strength (20%), and closeness to full coverage (15%) into a "Priority #N" chip per row, auditable via tooltip. Table stays ₹-sorted; a "Recommended closure order" line shows the priority-ranked sequence. Button relabeled "+ Add to closure plan"/"✓ In closure plan", badge "SIMULATION"→"PLAN PREVIEW", new efficiency-comparison sentence contrasts the user's pick against grabbing the largest ₹ gaps outright. Live-verified on NTPC: 15 priority chips render independent of table order; ticking 2 non-top-3 rows dropped Total Risk Exposure ₹367→₹313 Cr with correct PLAN PREVIEW notes and a real efficiency sentence ("captures 66%..."); Reset restored ₹367 Cr exactly; tour step 3 copy updated. Note: deployed `/app.js` needed a hard reload to bust client cache before the new code ran. Commit: 47e789a

## task-059 | 2026-07-12 | DONE
YC round-4 feedback: killed all-caps/JetBrains-Mono "AI terminal" chrome in favor of sentence-case Inter everywhere (code blocks still monospace via Consolas); plant dropdown now shows title-cased names + live per-plant doc counts + a guarded "Delete profile" option; `injectGraphLinkChips()` now also matches the user's question (not just the answer) so an explicit "show me the graph" ask renders a generic "Open the Risk & Gap Graph →" chip when no failure mode matches; graph physics now freezes (`stabilizationIterationsDone` → `physics:{enabled:false}`) instead of drifting forever. Live-verified: Inter font + sentence case confirmed via computed styles; dropdown showed "Ntpc · 4 docs"/"Saraighat · 3 docs" with working double-confirm-then-cancel on Delete; "show me the graph" produced the generic chip; graph canvas `toDataURL()` was byte-identical 2.5s apart post-load (no drift). Commit: 6000b5a (bundled with task-058).

## task-058 | 2026-07-12 | DONE
FEATURE_PLAN Tier-2 #3 (4-step demo/onboarding tour, zero deps, `?demo=1` / header "?" relaunch) and #5 (purposeful empty states for zero-doc plants, chat, and Live Sheet), a Drive-sync fallback for plants with no linked folder (opens upload panel with highlighted paste-a-folder-link instructions), and the fix for task-057's chat→graph chip-matching bug (both sides of the match now punctuation/hyphen-normalized). Live-verified: `?demo=1` auto-started and walked all 4 steps (hub→chat→sheet→graph) cleanly with no stray DOM after Finish; re-asking "What is the turbine high vibration SOP status?" now correctly rendered the "View ... in graph →" chip (task-057's bug fixed); Drive-sync fallback confirmed live (no-folder plant → sync button opens panel with `.dest-note--attn` highlighted note). Empty-state markup verified present via source read, not exercised against a real zero-doc plant this pass. Commit: 6000b5a.
